...
void AddFrameworks(ostream& os, const string& projName, const vector<string>& frameworks)
{
	...
	static const TMap frameworkHandlers = {{"opengl", handleOpenGL}, {"vulkan", handleVulkan}, {"cocoa", handleCocoa}};
		...
		else if (!groupDeps.empty())
		{
			for (const auto& d : dependencyList)
			{
				...
				if (isGrouped)
					...
					outsideGroup.push_back(d);
			}

			for (const auto& e : externalLibs)
				if (!e.empty())
					outsideGroup.push_back(e);
			for (const auto& l : libList)
				if (!l.empty())
					outsideGroup.push_back(l);
		}
		else
		{
			for (const auto& d : dependencyList)
