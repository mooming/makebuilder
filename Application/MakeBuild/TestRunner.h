...
public:
    ...
    static bool RunTests(const TString& testDir, const TString& buildDir);
    static void WriteResults(const TString& outputPath, const TestResults& results);
    static TString GetTimestamp();

private:
    static bool RunSingleTest(const TString& testPath, const TString& buildDir, TestResult& result);
